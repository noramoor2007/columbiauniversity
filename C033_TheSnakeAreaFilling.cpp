...
int snakefill (int x) { // This creates a function called "snakefill" that takes an integer value stored in "x" and return an integer value.
  int area = x * x; // The total number of squares is equal to the length squared. We know that the game has a field size of the input number squared, so why not make a variable for that.
  ...
  while (sum <= area) { // This while loop continues until the sum exceeds the maximum. It checks if the current sum (the length of the snake) is still within the game screen.
    ...
    if (sum <= area) { // If the sum is less than or equal to the maximum area.
